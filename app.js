...
function getTitle(c){
    return '<font size="4">Github </font><font size="4" id="' + c + '">Status</font>';
}

function getStatus(c, s){
    return '<center><b><font size="7" id="'+c+'">'+s.toUpperCase()+'</font></b></center>';
}
...
function Status(arr){
    ...
    if(arr.status == 'good'){
        image = svg('#2D8A54');
        colorID = 'green';
        //out = '<font size="7" id="'+colorID+'">GOOD</font>';
    } else if (arr.status == 'minor'){
        image = svg('#FFA500');
        colorID = 'orange';
        //out = '<font size="7" id="'+colorID+'">MINOR</font>';
    } else if (arr.status == 'major'){
        image = svg('#FF0000');
        colorID = 'red';
        //out = '<font size="7" id="'+colorID+'">MAJOR</font>';
    }
    document.getElementById("mainStatus").innerHTML =  getStatus(colorID, arr.status);
    document.getElementById("color").innerHTML =  getTitle(colorID);
